...
const getMonacoLanguageId = (apiLangCode: string) => {
    const map: Record<string, string> = {
        'c++': 'cpp', 'c#': 'csharp', 'python': 'python', 'javascript': 'javascript', 'java': 'java', 'go': 'go', 'rust': 'rust'
    };
...
const TryItYourselfRunner = ({ lessonId, onClose }: TryItYourselfRunnerProps) => {
  ...
  const [outputState, setOutputState] = useState({
    output: "", error: null as string | null, status: null as string | null, executionTime: 0
  });

  const shouldFetchConfig = !!lessonId && lessonId !== 'NaN';
  const { data: lessonConfig, isLoading: isConfigLoading } = useGetLessonTryItYourselfQuery(lessonId, { skip: !shouldFetchConfig });
  ...
  const handleReset = () => {
    if (lessonConfig) setCode(lessonConfig.example_code);
    else setCode(DEFAULT_CODE_SNIPPETS[getMonacoLanguageId(selectedLangCode)] || "");
    setOutputState({ output: "", error: null, status: null, executionTime: 0 });
  };

  if (isConfigLoading || isLanguagesLoading) return <div className="p-4"><Skeleton active paragraph={{ rows: 10 }} /></div>;

  const languageOptions = allLanguages?.map(lang => ({ value: lang.code, label: lang.name })) || [];
    ...
    <div className="flex flex-col h-full bg-[#1e1e1e]">
      ...
      <div className="flex justify-between items-center px-4 h-10 bg-[#262626] border-b border-white/5 shrink-0">
        <div className="flex items-center gap-2">
            <span className={`text-xs font-bold ${!lessonConfig ? 'text-emerald-500' : 'text-gray-400'}`}>
                &lt;/&gt; Code
            </span>
        ...
        <div className="flex items-center gap-2">
            {!lessonConfig && (
                <Tooltip title="Reset Code">
                    <Button type="text" size="small" icon={<ReloadOutlined />} onClick={handleReset} className="text-gray-500 hover:text-white" />
                </Tooltip>
            )}
         ...
         {isConsoleOpen && (
            <div className="h-[40%] border-t border-white/10 bg-[#1e1e1e] flex flex-col animate-slide-up">
                <div className="flex items-center justify-between px-4 py-2 bg-[#262626] border-b border-white/5 h-9 shrink-0">
                    <span className="text-xs font-bold text-gray-300">Testcase / Result</span>
                    <Button 
                        type="text" 
                        size="small" 
                        icon={<DownOutlined />} 
      ...
      <div className="h-12 bg-[#262626] border-t border-white/10 px-4 flex justify-between items-center shrink-0 z-10">
          ...
          >
             Console {isConsoleOpen ? <DownOutlined className="text-[10px] ml-1" /> : <UpOutlined className="text-[10px] ml-1" />}
          </Button>
